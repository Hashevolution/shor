...
\paragraph{Assumption (Regev marginal).} Each measurement of Regev's circuit produces a vector $(k_1, \ldots, k_d) \in \{0, \ldots, Q-1\}^d$, where the \emph{marginal} distribution of each coordinate $k_i$ satisfies $k_i \approx j_i \cdot Q / r_{a_i}$ for some integer $j_i$ --- i.e., is statistically identical to Shor's single-base measurement distribution for base $a_i$.

\noindent
(Consistent with public summaries of Regev's algorithm but ignores potential joint correlations that Regev's lattice post-processing may exploit.)
...
\paragraph{Caveat.} If the Regev marginal assumption fails (e.g., if coordinates are jointly entangled in a way that distorts individual marginals), Theorem~\ref{thm:regev-c}'s empirical numbers do not transfer. A precise analysis of Regev's measurement marginals from the original paper is left to future work.
