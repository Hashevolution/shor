...
See \texttt{demo.py -{}-verify <N>} in the companion code to reproduce; per-noise-model breakdown for $N=77$ is in Appendix~\ref{app:tables}.
...
Our experiments are at $N \leq 437$ (small for cryptographic purposes) but with high statistical replication (500 trials per noise setup). Scaling to cryptographic $N$ requires no algorithmic change; the bottleneck is classical state-vector simulation. Hardware verification on existing NISQ devices is a natural next step.
