...
\paragraph{Caveat.} The numbers above assume each coordinate of Regev's measurement is independently Shor-distributed. Regev's actual joint constraint $\sum_i b_i k_i \approx 0 \pmod r$ would correlate the coordinates, potentially reducing per-coordinate $(C)$ efficiency. The qualitative noise-invariance of Theorem~\ref{thm:c-det} carries over to Regev's setting whenever per-coordinate $(C)$ succeeds (since Theorem~\ref{thm:c-det} is a per-coordinate statement and makes no joint assumption); only the \emph{efficiency} claim of part~(b) depends on the marginal Shor-likeness. Empirically validating against Regev's exact measurement distribution is left to future work.
