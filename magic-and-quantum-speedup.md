...
- **정식화 완료(명제 6, `experiments/oracle_tcount_magic.py`):** $M_2(|\psi_f\rangle)>0\iff f$ 비선형
  $\iff U_f$ 게이트분해가 비클리포드($T$) 게이트 요구 — 셋 다 $f$ 아핀에서만 0. 즉 **오라클이
  숨긴 magic = 게이트분해의 $T$-비용**. 비선형 ANF 단항식 수 $T_{\rm proxy}$와 $M_2$가 동반
  증가(Simon $0/0$ vs Shor $T_{\rm proxy}{=}4{\to}156,\ M_2{=}1.54{\to}5.23$).
...
- **통계적 예측식 — 유도 완료(명제 4·5, `marker_code_closed_form.py`).** 닫힌형
  $M_2(|{\rm flat}_W\rangle)=-\log_2(\tfrac1{M^4}\sum_x E(W\cap(W{\oplus}x)))$ ($E$=가법에너지), 랜덤
  $W$(Sidon, $M\ll2^{n/2}$)는 $M_2=\log_2(M^3/(7M-6))\approx2\log_2M-\log_27$. 유한-$N$ 보정은
  가법충돌로 하락($M^2/N$ 통제). 잔여: $M^2\gtrsim N$ 보정항 해석 전개.
- **선행조사 결과(필독, `magic-prior-art.md` §5c):** 부호↔magic은 3갈래로 *이미 점유* —
  (A) RM↔매직상태 증류·가중치열거자(PRX 2,041021; 2308.05152 등; *다른 의미*, 용어충돌 주의),
  (B) **Boolean 비선형성↔하이퍼그래프 상태 magic(2308.01886=Quantum 8,1351; 2602.23687) —
  최근접 이웃**, (C) Dicke/순열불변=구조적 support 평탄상태(2402.08551). (B)는 정의식·지표가
  스니펫으로 다중확인됨: **하이퍼그래프=위상 인코딩 $\prod C_e|{+}\rangle^n=\sum(-1)^f|x\rangle$,
  지표=nonquadraticity(RM(2)).** 따라서 (B)와는 반드시 **"위상·RM(2)" vs "support·RM(1)/아핀"**로
  차별화하고, 자기상관 객체 자체는 2605.05347의 $\Lambda$ 출처임을 명기할 것. 방어 가능한 *좁은
  잔여*는 **flat 마커상태 SRE 정확형 + 아핀⟺$A_W\in\{0,M\}$ 영점판정 + Grover 다중표시 적용 +
  최소거리 무력성 반례**뿐. ((B) 2편은 전문 대조 완료(2026-06-18): 위상 인코딩·nonquadraticity(RM(2))
  확정, support/자기상관 미취급 확인 — `magic-prior-art.md` §5c.)
