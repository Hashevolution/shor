...
- **선행조사 결과(필독, `magic-prior-art.md` §5c):** 부호↔magic은 3갈래로 *이미 점유* —
  ...
  (B) **Boolean 비선형성↔하이퍼그래프 상태 magic(2308.01886·2602.23687) — 최근접 이웃**,
  (C) Dicke/순열불변=구조적 support 평탄상태(2402.08551). 따라서 (B)와는 반드시
  **"위상 인코딩·RM(2)" vs "support 인코딩·RM(1)/아핀"**로 차별화하고, 자기상관 객체 자체는
  2605.05347의 $\Lambda$ 출처임을 명기할 것. 방어 가능한 *좁은 잔여*는
  **flat 마커상태 SRE 정확형 + 아핀⟺$A_W\in\{0,M\}$ 영점판정 + Grover 다중표시 적용 + 최소거리
  무력성 반례**뿐. 착수 전 (B) 2편 전문 확보로 위상 vs support 구분 재확인 필요.
