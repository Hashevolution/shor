...
The §3.6 stochastic-resonance observation sits *orthogonal* to this main framework: at sub-Regev-optimal parallelism `d` (most prominently `d = 1`), a small phase noise `σ ≈ 0.05` produces a ~1-3% reduction in run count — a *weak* noise-as-resource effect bounded by the "anti-optimization" caveat (only accessible when one has already sacrificed the parallelism gain of larger `d`). It does not interact with Theorems 1-5 in the regime where they are tight.
...
Additionally (§3.6), at sub-Regev-optimal parallelism a small phase noise produces a ~1-3% reduction in run count — a weak instance of *stochastic resonance* in multi-base quantum factoring under what we call the "anti-optimization" caveat: the effect coexists only with sacrificed efficiency, so it does not translate into a cryptographic implication, but it connects integer factoring to the broader *noise-as-resource* literature (ENAQT; Plenio–Huelga 2008).
